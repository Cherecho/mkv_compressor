...
Examples:
  ...
  mkv-compressor --list-presets
    ...
    parser.add_argument(
        'input',
        nargs='*',
        help='Input video file(s) or pattern (e.g., *.mp4)'
    )
    ...
    compression_group.add_argument(
        '--audio-bitrate',
        help='Audio bitrate (e.g., 128k, 192k)'
    )
    compression_group.add_argument(
        '--resolution',
    ...
    parser.add_argument(
        '--info',
        help='Show information about a video file and exit'
    )
    ...
    for path in paths:
        ...
        if path_obj.is_file():
            ...
            for match in matches:
                ...
                if match_path.is_file() and match_path.suffix.lower() in video_extensions:
        ...
        for input_file in video_files:
            ...
            try:
                ...
                if os.path.exists(output_file) and not args.overwrite:
                    print(f"Skipping {filename} - output exists (use --overwrite to replace)")
                    continue
