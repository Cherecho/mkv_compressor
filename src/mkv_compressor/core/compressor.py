...
class VideoCompressor:
    ...
    def compress_video(
        ...
        try:
            ...
            self.logger.info(f"Input video: {video_info.resolution}, {video_info.duration:.1f}s, {video_info.size_mb:.1f}MB")
            ...
            if settings.width and settings.height:
                video_stream = input_stream.video.filter('scale', settings.width, settings.height)
    ...
    def _build_pass1_command(self, input_path: str, settings: CompressionSettings) -> List[str]:
        ...
        try:
            ...
            while True:
                ...
                if process.poll() is not None:
                    ...
                    if buffer:
                        self._parse_ffmpeg_output(buffer.decode('utf-8', errors='ignore'), progress, pass_number)
                    break
    ...
    def _parse_ffmpeg_output(self, line: str, progress: CompressionProgress, pass_number: int):
                ...
                if time_match:
                    ...
                    if progress.total_duration > 0:
                        ...
                        if progress.is_two_pass:
                            if pass_number == 1:
                                final_progress = base_progress * 0.5  # First pass: 0-50%
                            else:
                                final_progress = 50 + (base_progress * 0.5)  # Second pass: 50-100%
                        else:
                            final_progress = base_progress
                        
                        # Clamp to reasonable range and update
                        final_progress = max(0, min(99, final_progress))  # Keep below 100% until truly done
                        ...
                        if 'speed=' in line:
                            ...
                            if speed_match:
                                ...
                                self.logger.debug(f"Compression speed: {speed}x (Progress: {final_progress:.1f}%)")
